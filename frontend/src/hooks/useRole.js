  ...
  useEffect(() => {
    ...
    async function detectRole() {
      ...
      try {
        ...
        const isMfg = await contract.hasRole(mfgRole, account);
        if (isMfg && isMounted) {
        ...
        const isDist = await contract.hasRole(distRole, account);
        if (isDist && isMounted) {
          setRole("distributor");
          setIsLoading(false);
          return;
        ...
        const isRet = await contract.hasRole(retRole, account);
        if (isRet && isMounted) {
          setRole("retailer");
          setIsLoading(false);
          return;
